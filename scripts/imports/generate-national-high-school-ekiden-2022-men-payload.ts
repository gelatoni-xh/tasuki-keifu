...
const ARCHIVE_URL = "https://koukouekiden-record.mainichi.jp/record/archive/m_73/all_record.html";
const SOURCE_ID = "source-mainichi-koko-ekiden-2022-men-archive";
...
async function main() {
  const outputDir = path.resolve(process.argv[2] ?? "data/imports/high-school-ekiden-2022-men");
  ...
  for (let legIndex = 0; legIndex < 7; legIndex += 1) {
    const payload = {
      batchKey: `national-high-school-ekiden-2022-men-leg-${legIndex + 1}`,
      sourceId: SOURCE_ID,
      raceSlug: `national-high-school-ekiden-2022-men-leg-${legIndex + 1}`,
      summary: `全国高校駅伝2022 男子 ${legIndex + 1}区`,
    ...
    await writeFile(
      path.join(outputDir, `national-high-school-ekiden-2022-men-leg-${legIndex + 1}.json`),

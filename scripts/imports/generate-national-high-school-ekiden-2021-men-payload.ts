...
const ARCHIVE_URL = "https://koukouekiden-record.mainichi.jp/record/archive/m_72/all_record.html";
const SOURCE_ID = "source-mainichi-koko-ekiden-2021-men-archive";
...
const SCHOOL_NAME_PREFECTURE_OVERRIDES = new Map<string, {
  ...
  ["西脇工::兵庫県", {
    ...
    canonicalNameJa: "西脇工業高校",
  }],
  ["東農大二::群馬県", {
    slug: "tokyo-nodai-daini-high-school",
    canonicalNameJa: "東京農業大学第二高校",
  }],
  ["西京::山口県", {
    slug: "hs-e5b1b1e58fa3e79c8ce7ab8b",
    canonicalNameJa: "山口県立西京高校",
  }],
  ["倉敷::岡山県", {
    slug: "kurashiki-high-school",
    canonicalNameJa: "倉敷高校",
  }],
  ["佐野日大::栃木県", {
    slug: "sano-nichidai-high-school",
    canonicalNameJa: "佐野日本大学高校",
  }],
  ["平田::島根県", {
    slug: "hirata-high-school",
    canonicalNameJa: "平田高校",
  }],
  ["十日町::新潟県", {
    slug: "tokamachi-high-school",
    canonicalNameJa: "十日町高校",
  }],
  ["つるぎ::徳島県", {
    canonicalNameJa: "つるぎ高校",
  }],
]);
...
function toCanonicalMark(value: string) {
  ...
  const missingDoubleQuoteMinuteMatch = normalized.match(/^(\d+)'(\d+)$/);
  if (missingDoubleQuoteMinuteMatch) {
    return `${missingDoubleQuoteMinuteMatch[1]}:${missingDoubleQuoteMinuteMatch[2].padStart(2, "0")}`;
  }
...
function parseRankCell(value: string) {
  ...
  const partialMatch = normalized.match(/^(\d+)\(--\)$/);
  if (partialMatch) {
    return {
      legRank: Number(partialMatch[1]),
      cumulativeRank: null,
    };
  }
...
function parseMarkCell(value: string) {
  ...
  const cumulativeMark = markMatch[2]
    ? (markMatch[2] === "--" ? null : toCanonicalMark(markMatch[2]))
    : legMark;
...
function parseTeamRows(html: string) {
  ...
  for (let index = 0; index < bodyRows.length; index += 3) {
    ...
    const normalizedCumulativeSnapshots = parsedRanks.map((item, legIndex) => {
      const legMark = parsedMarks[legIndex]?.legMark;
      if (legMark) {
        runningSeconds += markToSeconds(legMark);
      }
      return {
        cumulativeRank: item.cumulativeRank,
        cumulativeMark: parsedMarks[legIndex]?.cumulativeMark ?? (legMark ? secondsToMark(runningSeconds) : null),
      };
    });
...
function canonicalSchoolInfo(aliasMap: Map<string, HighSchoolLookupEntry[]>, schoolName: string, prefecture: string): CanonicalSchool {
  const override = SCHOOL_NAME_PREFECTURE_OVERRIDES.get(`${schoolName}::${prefecture}`);
  if (override) {
    return {
      slug: override.slug ?? null,
      canonicalNameJa: override.canonicalNameJa ?? schoolName,
      sourceNameJa: schoolName,
      prefecture,
      matchedBy: override.slug ? "slug" : "generated",
    };
  }
...
async function main() {
  const outputDir = path.resolve(process.argv[2] ?? "data/imports/high-school-ekiden-2021-men");
  ...
  for (let legIndex = 0; legIndex < 7; legIndex += 1) {
    const payload = {
      batchKey: `national-high-school-ekiden-2021-men-leg-${legIndex + 1}`,
      sourceId: SOURCE_ID,
      raceSlug: `national-high-school-ekiden-2021-men-leg-${legIndex + 1}`,
      summary: `全国高校駅伝2021 男子 ${legIndex + 1}区`,
      ...
      teamResults: rows.map((row) => {
        ...
        const cumulative = row.cumulativeSnapshots[legIndex];
        const schoolSlug = school.slug ?? slugifySchoolName(school.canonicalNameJa);
        const snapshotNotes = legIndex === 0 && school.canonicalNameJa === "つるぎ高校"
          ? "SDS / 区間未発走"
          : "都道府県代表";
    ...
    await writeFile(
      path.join(outputDir, `national-high-school-ekiden-2021-men-leg-${legIndex + 1}.json`),

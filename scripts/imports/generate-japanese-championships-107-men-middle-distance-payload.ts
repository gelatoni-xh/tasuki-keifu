...
const SOURCE_ID = "source-jch-107-result";
const SOURCE_URL = "https://www.jaaf.or.jp/files/upload/202306/04_201132.pdf";
const BATCH_DATE = "20260710";
const SOURCE_DIR = path.resolve("data/imports/japanese-championships-5000m-107-source");
...
const LOCAL_CACHE_PDF_PATH = path.resolve("data/imports/jch-2023-resultall.pdf");
...
const SECTION_DEFINITIONS: SectionDefinition[] = [
  { key: "800-heat-1", raceSlug: "japanese-championships-5000m-107-men-800-heat-1", sourceEntityPrefix: "jch107-men-800-heat1", title: "男子800m 予選1組", discipline: "m800" },
  { key: "800-heat-2", raceSlug: "japanese-championships-5000m-107-men-800-heat-2", sourceEntityPrefix: "jch107-men-800-heat2", title: "男子800m 予選2組", discipline: "m800" },
  { key: "800-heat-3", raceSlug: "japanese-championships-5000m-107-men-800-heat-3", sourceEntityPrefix: "jch107-men-800-heat3", title: "男子800m 予選3組", discipline: "m800" },
  { key: "800-final", raceSlug: "japanese-championships-5000m-107-men-800-final", sourceEntityPrefix: "jch107-men-800-final", title: "男子800m 決勝", discipline: "m800" },
  { key: "1500-heat-1", raceSlug: "japanese-championships-5000m-107-men-1500-heat-1", sourceEntityPrefix: "jch107-men-1500-heat1", title: "男子1500m 予選1組", discipline: "m1500" },
  { key: "1500-heat-2", raceSlug: "japanese-championships-5000m-107-men-1500-heat-2", sourceEntityPrefix: "jch107-men-1500-heat2", title: "男子1500m 予選2組", discipline: "m1500" },
  { key: "1500-final", raceSlug: "japanese-championships-5000m-107-men-1500-final", sourceEntityPrefix: "jch107-men-1500-final", title: "男子1500m 決勝", discipline: "m1500" },
  { key: "3000sc-final", raceSlug: "japanese-championships-5000m-107-men-3000sc-final", sourceEntityPrefix: "jch107-men-3000sc-final", title: "男子3000mSC 決勝", discipline: "m3000sc" },
];
...
const ORGANIZATION_ENTITY_OVERRIDES = new Map<string, { slug: string; nameJa: string; type: OrgType }>([
  ...
  ["TAKEOｱｽﾚﾁｯｸｸﾗﾌﾞ", { slug: "takeo-athletic-club", nameJa: "TAKEOアスレチッククラブ", type: "club" }],
  ["MORE", { slug: "more", nameJa: "MORE", type: "club" }],
  ["GANAR", { slug: "ganar", nameJa: "GANAR", type: "club" }],
  ["Astray", { slug: "astray", nameJa: "Astray", type: "club" }],
  ...
  ["北海道大大学院", { slug: "hokkaido-university-graduate-school", nameJa: "北海道大学大学院", type: "university" }],
  ["筑波大大学院", { slug: "org-hokuren2023shibetsu-b34219d32a23ec3390a0", nameJa: "筑波大学大学院", type: "university" }],
  ["埼玉医科大G", { slug: "saitama-medical-university-group", nameJa: "埼玉医科大学グループ", type: "corporate_team" }],
  ["Resistance.Sol", { slug: "resistance-sol", nameJa: "Resistance.Sol", type: "club" }],
]);
...
function inferOrganizationType(label: string): OrgType {
  ...
  if (/(AC|RC|FC|TC|クラブ|陸協|Team|Sol|MORE|GANAR)$/i.test(normalized)) return "club";
  return "corporate_team";
}
...
function parseSingleColumnLine(line: string) {
  ...
  const matched = compacted.match(
    /^(?:(\d+)\s+)?(\d+)\s+([A-Z0-9]+)\s+(.+?\(\d+\))\s+(.+?)\s+(DNS|DNF|DQ(?:,[A-Za-z0-9]+)?|\d{1,2}:\d{2}\.\d{2})(?:\s+([A-Za-z0-9,]+))?$/,
  );
  ...
  const notes =
    mark === "DNS" || mark === "DNF" || mark === "DQ"
      ? inlineRemark || remark
        ? `${mark},${[inlineRemark, remark].filter(Boolean).join(",")}`
        : mark
      : remark ?? null;
  ...
  return {
    ...
    organizationLabel,
    mark,
    rank: mark === "DNS" || mark === "DNF" || mark === "DQ" ? null : place ? Number(place) : null,
    notes,
  } satisfies ParsedRow;
}
...
function extractRowLines(block: string) {
  return block
    ...
    .filter((line) => /^\s*(?:\d+\s+)?\d+\s+[A-Z0-9]+\s+\S/.test(line));
}
...
function parseDualColumnBlock(block: string) {
  ...
  for (const line of block.split(/\r?\n/)) {
    ...
    if (/^\s*(?:\d+\s+)?\d+\s+[A-Z0-9]+\s+\S/.test(left)) leftRows.push(parseSingleColumnLine(left));
    if (/^\s*(?:\d+\s+)?\d+\s+[A-Z0-9]+\s+\S/.test(right)) rightRows.push(parseSingleColumnLine(right));
  }
...
function resolvePersonSlug(name: string, lookups: Awaited<ReturnType<typeof loadPersonLookups>>) {
  ...
  return fallbackSlug("person-jch107-mid", name);
}
...
function resolveOrganization(label: string, lookup: Awaited<ReturnType<typeof loadOrganizationLookup>>) {
  ...
  return { slug: fallbackSlug("org-jch107-mid", normalized), nameJa: normalized, type: inferOrganizationType(normalized) } satisfies ResolvedOrganization;
}
...
async function buildPayload(
  ...
  return raceImportPayloadSchema.parse({
    ...
    summary: `第107回日本陸上競技選手権大会 ${section.title} JAAF公式結果PDF import`,
    pbNotes: "JAAF公式結果PDFでは男子800m・男子1500m・男子3000mSCにPBコメントがあるため、PBコメント付きの結果のみPB反哺を行う。",
    entries,
  });
}

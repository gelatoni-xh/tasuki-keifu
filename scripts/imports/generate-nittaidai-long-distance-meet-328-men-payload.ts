...
const SOURCE_ID = "source-nittaidai-long-distance-meet-328-results";
const BASE_URL = "https://nittai-honbu.pecori.jp/nittai-result/328042526/shtml/";
const TAIKAI_URL = `${BASE_URL}Taikai.json`;
const TIMETABLE_URL = `${BASE_URL}TimeTable.json`;
const BATCH_DATE = "20260713";
const SOURCE_DIR = path.resolve("data/imports/nittaidai-long-distance-meet-328-source");
...
const PERSON_SLUG_OVERRIDES = new Map<string, string>([
  ["桑原 大地", "person-nittaidai328-ac0a0777128b13477117"],
  ["桑原大地", "person-nittaidai328-ac0a0777128b13477117"],
...
const PERSON_ORG_SLUG_OVERRIDES = new Map<string, string>([
  ["生駒蓮::tokyo-international-university", "person-nittaidai328-dac6d2f38c6a4738ea7a"],
]);
...
function resolveOrganization(label: string, lookup: Awaited<ReturnType<typeof loadOrganizationLookup>>) {
  ...
  return {
    slug: fallbackSlug("org-nittaidai328", normalizedLabel),
...
function resolvePersonSlug(displayNameJa: string, organizationSlug: string, lookup: Awaited<ReturnType<typeof loadPersonLookup>>) {
  ...
  if (candidates.length === 0) {
    return `person-nittaidai328-${createHash("sha1").update(`${organizationSlug}|${displayNameJa}`).digest("hex").slice(0, 20)}`;
  }
...
function toRaceDefinition(row: TimeTableRow) {
  ...
  if (isNcg && distance === "5000" && groupIndex === 2) {
    return {
      raceSlug: "nittaidai-long-distance-meet-328-ncg-men-5000-heat-2",
      displayName: "NCG男子5000m 2組",
      discipline,
      sourceEntityPrefix: "nittaidai-328-ncg-men-5000-2",
      resultUrl,
      resultAnchor,
      groupIndex,
  ...
  if (isNcg && distance === "1500" && groupIndex === 2) {
    return {
      raceSlug: "nittaidai-long-distance-meet-328-ncg-men-1500-heat-2",
      displayName: "NCG男子1500m 2組",
      discipline,
      sourceEntityPrefix: "nittaidai-328-ncg-men-1500-2",
      resultUrl,
      resultAnchor,
      groupIndex,
    } satisfies RaceDefinition;
  ...
  if (isNcg) {
    return {
      raceSlug: `nittaidai-long-distance-meet-328-ncg-men-${distance}`,
      displayName: row.KyogiMei,
      discipline,
      sourceEntityPrefix: `nittaidai-328-ncg-men-${distance}`,
      resultUrl,
      resultAnchor,
      groupIndex,
    } satisfies RaceDefinition;
  ...
  return {
    raceSlug: `nittaidai-long-distance-meet-328-men-${distance}-heat-${groupIndex}`,
    displayName: `${row.KyogiMei} ${groupIndex}組`,
    discipline,
    sourceEntityPrefix: `nittaidai-328-men-${distance}-${groupIndex}`,
    resultUrl,
    resultAnchor,
    groupIndex,
  } satisfies RaceDefinition;
...
async function buildPayload(definition: RaceDefinition, organizationLookup: Awaited<ReturnType<typeof loadOrganizationLookup>>, personLookup: Awaited<ReturnType<typeof loadPersonLookup>>) {
  ...
  return raceImportPayloadSchema.parse({
    ...
    summary: `第328回日本体育大学長距離競技会兼21st NITTAIDAI Challenge Games ${definition.displayName} PECORI公式結果JSON import`,

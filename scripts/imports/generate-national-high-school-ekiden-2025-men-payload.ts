...
const RESULT_PDF_URL = "https://www.jaaf.or.jp/files/upload/202512/21_150042.pdf";
const RESULT_SOURCE_ID = "source-jaaf-koko-ekiden-2025-men-result";
const EDITION_SLUG = "national-high-school-ekiden-2025-men";
const EDITION_LABEL = "全国高校駅伝2025 男子";
...
const EXPECTED_PREFECTURES = [
  ...
  "埼玉県", "千葉県", "東京都", "神奈川県", "山梨県", "新潟県", "富山県", "石川県", "福井県", "長野県",
...
const PREFECTURE_NORMALIZATION = new Map<string, string>([
  ["北海道", "北海道"], ["青森", "青森県"], ["岩手", "岩手県"], ["宮城", "宮城県"], ["秋田", "秋田県"], ["山形", "山形県"],
  ["福島", "福島県"], ["茨城", "茨城県"], ["栃木", "栃木県"], ["群馬", "群馬県"], ["埼玉", "埼玉県"], ["千葉", "千葉県"],
  ["東京", "東京都"], ["神奈川", "神奈川県"], ["山梨", "山梨県"], ["新潟", "新潟県"], ["富山", "富山県"], ["石川", "石川県"],
  ["福井", "福井県"], ["長野", "長野県"], ["岐阜", "岐阜県"], ["静岡", "静岡県"], ["愛知", "愛知県"], ["三重", "三重県"],
  ["滋賀", "滋賀県"], ["京都", "京都府"], ["大阪", "大阪府"], ["兵庫", "兵庫県"], ["奈良", "奈良県"], ["和歌山", "和歌山県"],
  ["鳥取", "鳥取県"], ["島根", "島根県"], ["岡山", "岡山県"], ["広島", "広島県"], ["山口", "山口県"], ["徳島", "徳島県"],
  ["香川", "香川県"], ["愛媛", "愛媛県"], ["高知", "高知県"], ["福岡", "福岡県"], ["佐賀", "佐賀県"], ["長崎", "長崎県"],
...
function parseGradeFromName(value: string) {
  ...
  if (!match) {
    return { displayNameJa: normalizeDisplayNameJa(value), grade: null };
  }
  return { displayNameJa: normalizeDisplayNameJa(match[1]), grade: Number(match[2]) };
}
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    try {
      await new Promise<void>((resolve, reject) => {
        const req = request(url, { headers: { "user-agent": "Mozilla/5.0" }, timeout: 20000 }, (response) => {
...
function resolvePersonSlug(personLookup: Map<string, PersonLookupEntry[]>, displayNameJa: string, schoolSlug: string, fallbackSeed: string) {
  const candidates = personLookup.get(normalizeDisplayNameJa(displayNameJa)) ?? [];
  const matched = candidates.find((candidate) => candidate.highSchoolSlugs.has(schoolSlug) || candidate.raceOrganizationSlugs.has(schoolSlug));
...
function canonicalSchoolInfo(aliasMap: Map<string, HighSchoolLookupEntry[]>, schoolName: string, prefecture: string): CanonicalSchool {
  ...
  if (override?.slug) {
    for (const candidates of aliasMap.values()) {
      ...
      if (matched) {
        return { slug: matched.slug, canonicalNameJa: matched.nameJa, sourceNameJa: schoolName, prefecture, matchedBy: "slug" };
      }
    }
  }

  if (override?.canonicalNameJa) {
    return { slug: null, canonicalNameJa: override.canonicalNameJa, sourceNameJa: schoolName, prefecture, matchedBy: "generated" };
  }
  ...
  return { slug: null, canonicalNameJa, sourceNameJa: schoolName, prefecture, matchedBy: "generated" };
}

function parseStageLine(line: string) {
  const cleaned = line.replace(/^(?:NHB|NGR)\s+/, "");
  const matches = [...cleaned.matchAll(/\(\s*(\d+)\)\s+(?:\*\s*)?([0-9:]+)\s*/g)];
  return matches.map((match) => ({ rank: Number(match[1]), mark: match[2] }));
}

function extractPrefectureLabelFromLine(line: string) {
  const cleaned = line.replace(/^(?:NHB|NGR)\s+/, "");
  const trimmed = normalizeWhitespace(cleaned);
  const compactLine = compact(cleaned);
  const sortedLabels = [...new Set([...EXPECTED_PREFECTURES, ...PREFECTURE_NORMALIZATION.keys(), ...REGION_TO_PREFECTURE.keys()])]
    .sort((left, right) => right.length - left.length);

  for (const label of sortedLabels) {
    const compactLabel = compact(label);
    if (compactLabel && compactLine.startsWith(compactLabel)) {
      return label;
    }
  }
...
function parseTotalPages(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (runnerTokens.length !== 7) continue;
    ...
    if (cumulativeSnapshots.length !== 7 || legSnapshots.length !== 7) continue;
    ...
    rows.push({
      ...
      cumulativeSnapshots: cumulativeSnapshots.map((snapshot) => ({ cumulativeRank: snapshot.rank, cumulativeMark: snapshot.mark })),
      legSnapshots: legSnapshots.map((snapshot) => ({ legRank: snapshot.rank, legMark: snapshot.mark })),
    });
...
function assert47PrefectureCoverage(rows: TeamPerformanceRow[]) {
  const prefectures = [...new Set(rows.filter((row) => row.representativeType === "prefecture").map((row) => row.prefecture))]
...
async function main() {
  const outputDir = path.resolve(process.argv[2] ?? "data/imports/high-school-ekiden-2025-men");
  ...
  for (let legIndex = 0; legIndex < 7; legIndex += 1) {
    const payload = {
      batchKey: `${EDITION_SLUG}-leg-${legIndex + 1}`,
      sourceId: RESULT_SOURCE_ID,
      raceSlug: `${EDITION_SLUG}-leg-${legIndex + 1}`,
      summary: `${EDITION_LABEL} ${legIndex + 1}区`,
      pbNotes: "No PB snapshot on source",
      entries: rows.map((row) => {
        const school = schoolInfoByName.get(row.schoolName);
        if (!school) throw new Error(`Missing school info for ${row.schoolName}`);
    ...
    await writeFile(path.join(outputDir, `${EDITION_SLUG}-leg-${legIndex + 1}.json`), JSON.stringify(payload, null, 2));
  }

...
async function main() {
  const baseDir = path.resolve(process.argv[2] ?? "data/imports/high-school-ekiden-2021-men");

  let updatedGapCount = 0;
  let updatedAwardCount = 0;

  for (let leg = 1; leg <= 7; leg += 1) {
    const payloadPath = path.join(baseDir, `national-high-school-ekiden-2021-men-leg-${leg}.json`);

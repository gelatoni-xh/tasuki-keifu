...
async function main() {
  const payloadPath = path.resolve(
    process.argv[2] ?? "data/imports/high-school-ekiden-2024-men/national-high-school-ekiden-2024-men-leg-7.json",
  );
  ...
  if (!edition) {
    throw new Error("Missing edition national-high-school-ekiden-2024-men");
  }

  const payloadByOrganizationSlug = new Map(
    payload.teamResults.map((teamResult) => [teamResult.organizationSlug, teamResult] as const),
  );
  const payloadByOrganizationName = new Map(
    payload.teamResults
      .filter((teamResult) => teamResult.organizationNameJa)
      .map((teamResult) => [teamResult.organizationNameJa!, teamResult] as const),
  );
  ...
  for (const teamResult of edition.teamCompetitionResults) {
    const source =
      ...
      payloadByCanonicalKey.get(
        buildOrganizationCanonicalKey(teamResult.organization.nameJa, OrganizationType.high_school),
      );
    if (!source) {
      continue;
    }
